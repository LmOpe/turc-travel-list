...
function PackingLIst({ items, onDeleteIem, onToggleItem, onClearList }) {
  const [sortBy, setSortBy] = useState('input');

  let sortedItems;
  if (sortBy === 'input') sortedItems = items;
  if (sortBy === 'description') sortedItems = items
    .slice().sort((a, b) => a.description
...
function Item({ item, onDeleteIem, onToggleItem }) {
  return (
    <li>
      <input
        type="checkbox"
...
function Stats({ items }) {
  ...
  return (
    <footer className="stats">
      <em>
        {percentPacked === 100 ? "You got everything ready to go ✈" : `💼 You have ${numItems} items on your list, and you already packed ${numPackedItems} (${percentPacked}%)`}
